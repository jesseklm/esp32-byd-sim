...
void CanManager::readMessage() {
  ...
  if (rxId == 0x91) {
    ...
    const auto wr_battery_temperature = getValue<uint16_t>(rxBuf, 4);
    ...
    MqttManager::publish("inverter/battery_temperature", static_cast<float>(wr_battery_temperature) * 0.1f);
  } else if (rxId == 0xd1) {
